...
describe('validateTripData: success cases', () => {
  ...
  it('accepts antimeridian-crossing bounds without complaint', () => {
    ...
    expect(
      validateTripData(buildTripData({ items: [item], bounds: [179.9, -55, -179.9, -54] })),
    ).toEqual([]);
  });
  ...
  it('accepts equal consecutive timestamps (non-decreasing, not strictly increasing)', () => {
    ...
    expect(
      validateTripData(buildTripData({ items: [buildTrackItem({ geometries: [line] })] })),
    ).toEqual([]);
  });
});
...
describe('validateTripData: semantic failures', () => {
  it('reports mismatched per-point attribute array lengths', () => {
    ...
    const issues = validateTripData(
      buildTripData({ items: [buildTrackItem({ geometries: [line] })] }),
    );
    expect(issues.some((i) => i.message.includes('speed') && i.message.includes('length'))).toBe(
      true,
    );
  });
  ...
  it('reports duplicate item ids', () => {
    ...
    expect(issues.some((i) => i.message.includes('duplicate') && i.message.includes('id'))).toBe(
      true,
    );
  });
  ...
  it('reports duplicate order values', () => {
    ...
    const issues = validateTripData(buildTripData({ items: [a, b] }));
    expect(issues.some((i) => i.message.includes('duplicate') && i.message.includes('order'))).toBe(
      true,
    );
  });
  ...
  it('reports an unclosed polygon ring', () => {
    ...
    const issues = validateTripData(
      buildTripData({ items: [buildTrackItem({ geometries: [polygon] })] }),
    );
  ...
  it('reports an impossible calendar date that matches the pattern', () => {
    const issues = validateTripData(
      buildTripData({ items: [buildTrackItem({ day: '2030-02-30' })] }),
    );
