...
// Downloaded from: https://www.gpsvisualizer.com/leaflet/functions.js
		...
		if (segment_points[s] && segment_points[s].length > 0) {
			...
			if (trk[tn].info.z_index) {
				if (parseFloat(trk[tn].info.z_index) > 0) {
					lastItem(trk[tn].overlays).bringToFront(); // stopgap until leaflet supports zIndex; doesn't really work though
				} else {
...
function GV_Toggle_Overlays(overlay_array,force) {
	...
	if (overlay_array.gv_hidden_by_click) {
		...
		if (gvg.z_index) { GV_Restack_Tracks(); }
...
function GV_Restack_Tracks() {
	if (self.trk && Object.keys(trk).length > 0) {
		var z_index = []; var z_sort = false; // stopgap until leaflet supports zIndex: sort tracks by zIndex and bring them to the front one by one
		for (var j in trk) {
			z_index[j] = parseFloat(j)/100000; if (isNumeric(trk[j].info.z_index)) { z_index[j] = trk[j].info.z_index; z_sort = true; }
		}
		if (z_sort) {
			var tkeys = Object.keys(trk);
			tkeys.sort(function(a,b){return(z_index[a]-z_index[b]);});
			for (var k=0; k<tkeys.length; k++) {
				var tn = tkeys[k];
				for (var o=0; o<trk[tn].overlays.length; o++) { trk[tn].overlays[o].bringToFront(); }
			}
			gvg.z_index = true;
		}
	}
...
function GV_Load_Markers_From_File(file_index) {
	...
	if (gvg.marker_cluster && gv_options.marker_clustering) {
		GV_Uncluster_Dynamic_Markers();
	}
...
function GV_Load_Markers_From_Data_Object(data) {
	...
	if (root_tag == 'kml') { // it's a KML file
		...
		if (data && data[root_tag] && data[root_tag]['metadata']) {
			if (!gvg.dynamic_metadata) { gvg.dynamic_metadata = []; }
			gvg.dynamic_metadata = data[root_tag]['metadata'];
		}
	...
	gv_options.autozoom_default = (typeof(opts.zoom_default) != 'undefined') ? opts.zoom_default : gv_options.autozoom_default;
...
function GV_Recenter_Per_URL(opts) {
	...
	if (new_center) {
		...
		if (open_info_window && gvg.found_marker_index != null && wpts[gvg.found_marker_index]) {
			window.setTimeout('wpts['+gvg.found_marker_index+'].fire("click"); wpts['+gvg.found_marker_index+'].fire("mouseout")',500);
		}
...
function GV_Maptype_From_URL(opts) {
	if (!opts) { return null; }
	var mt = null;
	var maptype_key = (opts.maptype_key) ? opts.maptype_key : '(?:map.?type|bg.?map|base.?map|bg)';
	var type_aliases = (opts.type_alias) ? opts.type_alias : null;
...
function GV_Autozoom() { // automatically adjust the map's zoom level to cover the elements passed to this function; the first argument MUST be an options array
	...
	o.outlier_z = (gv_options.autozoom_outlier_z) ? parseFloat(gv_options.autozoom_outlier_z) : parseFloat(o.outlier_z);
	if (o.default_zoom !== null) {
	...
	if (o.outlier_z) { // if a critical Z was given, autozoom will be based only on the wpts array
		var sum_lat=0, sum_lon=0, sum_of_squared_lats=0, sum_of_squared_lons=0, count_of_coords=0;
		for (var j in wpts) {
			count_of_coords++; var lat = wpts[j].getLatLng().lat; var lon = wpts[j].getLatLng().lng;
			sum_lat += lat; sum_of_squared_lats += lat*lat; sum_lon += lon; sum_of_squared_lons += lon*lon;
		}
		var mean_lat = sum_lat/count_of_coords; var ss_lat = sum_of_squared_lats - ((sum_lat*sum_lat)/count_of_coords);
		var mean_lon = sum_lon/count_of_coords; var ss_lon = sum_of_squared_lons - ((sum_lon*sum_lon)/count_of_coords);
		var var_lat = (count_of_coords > 1) ? ss_lat/(count_of_coords-1) : 0; var var_lon = (count_of_coords > 1) ? ss_lon/(count_of_coords-1) : 0;
		var sd_lat = (var_lat > 0) ? Math.sqrt(var_lat) : 0; var sd_lon = (var_lon > 0) ? Math.sqrt(var_lon) : 0;
		objects = [];
		for (var j in wpts) {
			var lat = wpts[j].getLatLng().lat; var lon = wpts[j].getLatLng().lng;
			if (Math.abs((lat-mean_lat)/sd_lat) <= Math.abs(o.outlier_z) && Math.abs((lon-mean_lon)/sd_lon) <= Math.abs(o.outlier_z)) { objects.push(wpts[j]); }
		}
		if (objects.length == 0) { objects = wpts; } // oh well
	} else if (o.filter) {
		var field, text;
	...
	az_loop1:for (var i=0; i<objects.length; i++) {
		if (objects[i]) {
			var a = (typeof(objects[i]) == 'string') ? eval(objects[i]) : objects[i]; // so that 'wpts' is a valid argument
			if (a.getLatLng) { // it's a waypoint
				if (a.gvi && a.gvi.exclude_from_autozoom) { continue az_loop1; }
				if (!o.visible || !a.gv_hidden_manually()) {
					bounds.extend(a.getLatLng());
				}
			} else if (typeof(a) == 'object' && a.overlays && a.overlays.length && a.overlays.length) { // tracks are collections of overlays (usually track segments but also sometimes markers)
				if (a.overlays && a.overlays[0] && a.overlays[0].getLatLngs) { // it's a single track
					if (a.info && a.info.exclude_from_autozoom) { continue az_loop1; }
					if (!o.visible || !a.gv_hidden_manually()) {
				...
				az_loop2:for (var j=0; j<a.length; j++) { // it's a collection of tracks or markers
					if (a[j]) {
						if (a[j].getLatLng) { // it's a waypoint
							if (a[j].gvi && a[j].gvi.exclude_from_autozoom) { continue az_loop2; }
							...
							if (a[j].info && a[j].info.exclude_from_autozoom) { continue az_loop2; }
							if (!o.visible || !a[j].gv_hidden_manually()) {
	...
	if (bounds) {
		if (!bounds.isValid()) {
			...
			if (bounds.getNorthEast().lat == bounds.getSouthWest().lat && bounds.getNorthEast().lng == bounds.getSouthWest().lng) {
				if (o.default_zoom === null) {
					gvg.zoom = o.adjustment + gmap.getZoom();
				} else {
	...
	if (mapinfo.url || mapinfo.background) {
		if (mapinfo.url) {
			...
			for (var u=0; u<mapinfo.url_count; u++) {
				...
				var wms = (mapinfo.type == 'wms' || (mapinfo.type != 'tiles' && url.match(/\b(service=WMS|[cs]rs=EPSG:\d+|request=GetMap|WMSServer|bboxSR)\b/i))) ? true : false;
...
function GV_MapTypeControl(placement,add_help_link) {
	...
	if (gvg[selector_id]) { // if it's already been created before, don't rebuild it; just select the proper map type
		...
		var custom_orders = {};
		if (o.custom_order) {
			for (var id in o.custom_order) { var normalized_id = gvg.bg[ id ]; custom_orders[normalized_id] = true; }
		}
		var sorted_maps = [];
		...
		for (var j=0; j<sorted_maps.length; j++) {
			...
			if (map_ok && o.filter && !custom_orders[normalized_id] && gmap.getCenter && m.bounds && m.bounds.length >= 4) {
				var b = m.bounds;
...
function GV_Utilities_Menu(show) {
	if (show !== false) {
		if (typeof(gv_options.utilities_menu) != 'object') { gv_options.utilities_menu = { 'maptype':true, 'opacity':true, 'measure':true, 'link':true, 'profile':true, 'export':true, 'geolocate':true, 'profile':true }; }
		...
		if (gv_options.utilities_menu['measure'] !== false || gv_options.utilities_menu['export'] !== false || gv_options.utilities_menu['geolocate'] !== false || gv_options.utilities_menu['profile'] !== false || 1==1) {
			...
			if (gv_options.utilities_menu['link'] !== false) {
				html += '	<div class="gv_utilities_menu_item" id="gv_utilities_menu_link"><a href="javascript:void(0)" onclick="GV_Current_View_Link(); GV_Utilities_Menu(false);"><img src="'+gvg.embedded_images['utilities-link']+'" width="15" height="15" border="0" />Get link to current view</a></div>';
			}
...
function GV_Current_View_Link() {
	var use_hash = (gv_options.centering_options && gv_options.centering_options.hash) ? true : false; var hash = document.location.hash.replace(/^#/,'');
	var url = document.location.toString().replace(/#.*$/,''); var qs = [];
	var ck = (gv_options.centering_options && gv_options.centering_options.center_key) ? gv_options.centering_options.center_key : 'center'; var cre = new RegExp('\\b'+ck+'=[^&]*&?'); url = url.replace(cre,''); url = url.replace(/&$/,''); hash = hash.replace(cre,''); hash = hash.replace(/&$/,'');
	var zk = (gv_options.centering_options && gv_options.centering_options.zoom_key) ? gv_options.centering_options.zoom_key : 'zoom'; var zre = new RegExp('\\b'+zk+'=[^&]*&?'); url = url.replace(zre,''); url = url.replace(/&$/,''); hash = hash.replace(zre,''); hash = hash.replace(/&$/,'');
	// var bk = (gv_options.centering_options && gv_options.centering_options.maptype_key) ? gv_options.centering_options.maptype_key : 'bg'; var bre = new RegExp('\\b'+bk+'=[^&]*&?'); url = url.replace(bre,'');
	qs.push(ck+'='+parseFloat(gmap.getCenter().lat.toFixed(6))+','+parseFloat(gmap.getCenter().lng.toFixed(6)));
	qs.push(zk+'='+gmap.getZoom());
	url = url.replace(/[\?&]+$/,'');
	if (use_hash) {
		hash += (hash) ? '&' : ''; hash += qs.join('&'); url += '#'+hash;
	} else {
		url += url.match(/\?/) ? '&' : '?'; url += qs.join('&'); url += hash;
	}
	var html = '<p>Use <a href="'+url.replace(/"/g,"&quot;")+'">this link</a> to return to the current viewport:</p>';
	html += '<input id="gv_view_link_box" type="text" style="box-sizing:border-box; width:100%; xmax-width:40em;" value="'+url.replace(/"/g,"&quot;")+'" onfocus="this.select();" /></div>';
	GV_Create_Modal_Window(html);
	window.setTimeout('if ($("gv_view_link_box")) { $("gv_view_link_box").focus(); $("gv_view_link_box").select(); }',100);
}

function GV_Setup_Clustering() {

	if (!self.L.markerClusterGroup) { gvg.marker_cluster = null; return false; }
	var mco = (gv_options.marker_clustering_options) ? gv_options.marker_clustering_options : [];
	mco.largeScale = (mco.largeScale) ? mco.largeScale : 1.25;
	mco.smallScale = (mco && mco.smallScale) ? mco.smallScale : 0.8;
	var mcir = mco.iconRadius = (mco && mco.iconRadius) ? mco.iconRadius : 20;
	mco.iconRadiusLarge = mcir * mco.largeScale;
	mco.iconRadiusSmall = mcir * mco.smallScale;
	var opts = { // defaults
		...
		,animate:true
		,polygonOptions:{color:'#009900',weight:2,opacity:0.8,fillColor:'#66cc66',fillOpacity:0.1}
		,iconCreateFunction:function(c){ var n=c.getChildCount(); var sz = ((n<10)?'small':(n<100)?'medium':'large'); var cn='marker-cluster marker-cluster-'+sz; var scale = ((n<10)?gvg.marker_clustering_options.smallScale:(n<100)?1:gvg.marker_clustering_options.largeScale); return L.divIcon({html:'<div><span>'+n+'</span></div>',className:cn,iconSize:[gvg.marker_clustering_options.iconRadius*scale*2,gvg.marker_clustering_options.iconRadius*scale*2]}); }
	};
	if (mco) { for (var j in mco) { opts[j] = mco[j]; } }
	gvg.marker_clustering_options = opts;
	...
	gvg.marker_cluster = null;
	gvg.marker_cluster = L.markerClusterGroup(opts);
	if (opts.spiderfyOnClick) {
		gvg.marker_cluster.on('clusterclick', function(a) { a.layer.spiderfy(); });
	}
...
function GV_Cluster_All_Markers() {
	var to_be_clustered = [];
	for (var j in wpts) {
		if (wpts[j] && !wpts[j].gvi.nocluster) {
			wpts[j].remove();
...
function GV_Uncluster_Dynamic_Markers() {
	if (!gvg.marker_cluster || !gv_options.marker_clustering) { return false; }
	var to_be_unclustered = [];
	for (var j in wpts) {
		if (wpts[j] && wpts[j].gvi.dynamic) { to_be_unclustered.push(wpts[j]); }
	}
	if (to_be_unclustered.length) { gvg.marker_cluster.removeLayers(to_be_unclustered); }
}
function GV_Disable_Clustering() { // using Leaflet.MarkerCluster.Freezable
	if (gvg.marker_cluster) {
		gvg.marker_cluster.disableClustering();
	}
}
function GV_Enable_Clustering() { // using Leaflet.MarkerCluster.Freezable
	if (gvg.marker_cluster) {
		gvg.marker_cluster.enableClustering();
	}
}
...
function GV_Create_Label(m,opts) {
	...
	var mouse = ''; if (gv_options.labels_clickable || opts.centered && opts.label_offset[1] < 0) { // only do this if there's a good chance the label is overlapping the marker itself  GV_Open_Marker_Window('+marker_name+');
...
function GV_Bearing() { // takes 2 LatLng objects OR 2 two-item arrays OR 4 numbers OR 2 marker names
	...
	var lat1 = coords[0].lat*Math.PI/180; var lon1 = coords[0].lng*Math.PI/180;
	var lat2 = coords[1].lat*Math.PI/180; var lon2 = coords[1].lng*Math.PI/180;
...
function GV_Polygon_Area(coords_array) {
	...
	if (coords_array.length > 2) {
		for (var i=0; i<(coords_array.length-1); i++) {
			area += ((coords_array[i+1].lng-coords_array[i].lng)*Math.PI/180) * (2+Math.sin(coords_array[i].lat*Math.PI/180)+Math.sin(coords_array[i+1].lat*Math.PI/180));
		}
...
GV_Geolocate.success = function(pos) {
	...
	if (o.marker) {
		...
		var i = GV_Draw_Marker({lat:coords.latitude,lon:coords.longitude,name:GV_Format_Date(timestamp)+' '+GV_Format_Time(timestamp),desc:coords.latitude.toFixed(6)+', '+coords.longitude.toFixed(6)+remove_link,color:c,icon:sym,scale:1,type:'geolocation',nolist:nl});
		gvg.geolocation_markers.push(i);

		if (o.marker_list && gvg.marker_list_exists) {
			GV_Marker_List();
		}
	}
...
GV_Geolocate.remove_markers = function() {
	if (!gvg.geolocation_markers || !gvg.geolocation_markers.length) { return false; }
...
function GV_Create_Modal_Window(html,click_anywhere,x,y,max_wd,max_ht) {
	...
	GV_Disable_Leaflet_Dragging(gvg.modal_div);
}
...
function GV_Define_Garmin_Icons(icon_dir,garmin_icon_set) {
	var garmin_codes = new Array(
		...
		,'Oil Field','Oval, Blue','Oval, Green','Oval, Red','Oval, Yellow'
		...
		,'Water','Water Hydrant','Water Source','Waypoint','Waypoint Flag','Weed Bed','Wrecker','Waterfowl','Winery'
		,'Zoo'
	...
	else if (garmin_icon_set == '48x48') { garmin_dir = icon_dir+'icons/garmin/48x48/'; }

	if (garmin_icon_set == '24x24') {
		garmin_codes.push(
			...
			,'Contact, Afro','Contact, Alien','Contact, Ball Cap','Contact, Big Ears','Contact, Biker','Contact, Blonde','Contact, Bug','Contact, Cat','Contact, Clown','Contact, Dog','Contact, Dreadlocks','Contact, Female1','Contact, Female2','Contact, Female3','Contact, Glasses','Contact, Goatee','Contact, Kung-Fu','Contact, Panda','Contact, Pig','Contact, Pirate','Contact, Ranger','Contact, Smiley','Contact, Spike','Contact, Sumo'
			,'Letter A, Blue','Letter A, Green','Letter A, Red','Letter B, Blue','Letter B, Green','Letter B, Red','Letter C, Blue','Letter C, Green','Letter C, Red','Letter D, Blue','Letter D, Green','Letter D, Red'
			,'Number 0, Blue','Number 0, Green','Number 0, Red','Number 1, Blue','Number 1, Green','Number 1, Red','Number 2, Blue','Number 2, Green','Number 2, Red','Number 3, Blue','Number 3, Green','Number 3, Red','Number 4, Blue','Number 4, Green','Number 4, Red','Number 5, Blue','Number 5, Green','Number 5, Red','Number 6, Blue','Number 6, Green','Number 6, Red','Number 7, Blue','Number 7, Green','Number 7, Red','Number 8, Blue','Number 8, Green','Number 8, Red','Number 9, Blue','Number 9, green','Number 9, Red'
		);
	}
...
function GV_Define_Embedded_Images() {
	...
	gvg.embedded_images['goto'] = 'data:image/png;base64,';
	gvg.embedded_images['goto1'] = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAJCAYAAADgkQYQAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAEVJREFUeNpiYWBgcGAgAFig9AE8ahxYkHlpaWn/kfmzZs1iBNFM2ATRARM+kzAUIStAN5EJXRdMAbpCQkEA9x1ehQABBgCXSxG3/Cu2yQAAAABJRU5ErkJggg==';
	gvg.embedded_images['goto2'] = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAJCAYAAADgkQYQAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAFJJREFUeNp0kNENACEIQy3DwkxM21MTDdazn+2jAeDubF2ZiSaKiJnZC6g+SG4TACvUM+wmNVUHpE0XVAFtNJ1aQAVtnTnMv51GbvUfrz99AgwALQMtn/hrpUAAAAAASUVORK5CYII=';
	...
	gvg.embedded_images['utilities-link'] = ' data:image/png;base64,';
	...
	gvg.embedded_images['tooltip_pointer'] = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAMAAAAFCAYAAACAcVaiAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAChJREFUeNpi+P//PwMIA8F/JhDJyMgI5jGCRBiggBFKwwVg4D9AgAEABI0K/Ssiq0YAAAAASUVORK5CYII=';
	gvg.embedded_images['tooltip_pointer2'] = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAHCAYAAAAvZezQAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAADBJREFUeNpi/P//PwMMMDIy/mdC5oAkmZA5YDYQ/0fWxgRRwIgiAFICF2REtwUgwADUORcJjfOnZwAAAABJRU5ErkJggg==';
}
...
function GV_Background_Map_List() {
	return [
		...
		,{ id:'OPENTOPOMAP', menu_order:2.20, menu_name:'OSM topo (OpenTopoMap)', description:'OpenTopoMap.org', credit:'Map data from <a target="_blank" href="https://www.opentopomap.org/">OpenTopoMap.org</a>', error_message:'OpenTopoMap tiles unavailable', min_zoom:1, max_zoom:17, url:'https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png' }
...
//		,{ id:'US_COUNTIES_HYBRID', menu_order:11.53, menu_name:'us: Counties+hybrid', description:'United States county outlines + hybrid', credit:'US Counties from <a target="_blank" href="https://tigerweb.geo.census.gov/tigerwebmain/TIGERweb_main.html">US Census Bureau</a>', error_message:'TIGERweb tiles unavailable', min_zoom:6, country:'us', bounds:[-169,18,-66,72], bounds_subtract:[-129.97,49.01,-66,90], bounds_subtract:[-129,49.5,-66,72], tile_size:256, url:['https://tigerweb.geo.census.gov/arcgis/services/TIGERweb/tigerWMS_Current/MapServer/WMSServer?service=WMS&request=GetMap&version=1.1.1&format=image/png&transparent=true&exceptions=application/vnd.ogc.se_inimage&srs=EPSG:4326&styles=&layers=3'], opacity:1, background:'GV_HYBRID' }
		...
		,{ id:'US_PUBLIC_OSM', menu_order:11.61, menu_name:'us: Public lands+OSM', description:'U.S. public lands with OpenStreetMap background', credit:'Public lands data from <a target="_blank" href="https://www.caltopo.com/">CalTopo.com</a>', error_message:'CalTopo tiles unavailable', min_zoom:4, max_zoom:14, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], background:'GV_OSM_RELIEF', foreground:'US_CALTOPO_LAND_OWNERSHIP', foreground_opacity:1.00 }
		,{ id:'US_PUBLIC_OSM2', menu_order:11.61*0, menu_name:'us: Public lands+OSM 2', description:'U.S. public lands with OpenStreetMap background', credit:'Public lands data from <a target="_blank" href="http://www.blm.gov/">BLM</a>', error_message:'map tiles unavailable', min_zoom:4, max_zoom:14, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], background:'GV_OSM_RELIEF', foreground:'US_BLM_LAND_OWNERSHIP', foreground_opacity:1.00 }
		,{ id:'US_PUBLIC_HYBRID', menu_order:11.62, menu_name:'us: Public lands+hybrid', description:'U.S. public lands with hybrid background', credit:'Public lands data from <a target="_blank" href="https://www.caltopo.com/">CalTopo.com</a>', error_message:'CalTopo tiles unavailable', min_zoom:4, max_zoom:14, country:'us', bounds:[-152,17,-65,65], bounds_subtract:[-129,49.5,-66,72], background:'GV_HYBRID', url:'https://gis.blm.gov/arcgis/rest/services/lands/BLM_Natl_SMA_Cached_without_PriUnk/MapServer/tile/{z}/{y}/{x}', opacity:0.4 }
		...
		,{ id:'US_CALTOPO_FIRES', menu_order:11.82*0, menu_name:'us: Fire Activity (CalTopo)', description:'Current wildfire activity in the U.S.', credit:'<a target="_blank" href="https://caltopo.com/">CalTopo.com<'+'/a>', error_message:'Fire data unavailable', min_zoom:5, max_zoom:14, country:'us', bounds:[-168,18,-52,68], bounds_subtract:[-129,49.5,-66,72], url:'https://caltopo.com/tile/modis_mp/{z}/{x}/{y}.png' }
		,{ id:'US_CALTOPO_FIRES_HYBRID', menu_order:11.82*0, menu_name:'us: Fire Activity (CalTopo)', description:'Current wildfire activity in the U.S.', credit:'<a target="_blank" href="https://caltopo.com/">CalTopo.com<'+'/a>', error_message:'Fire data unavailable', min_zoom:5, max_zoom:14, country:'us', bounds:[-168,18,-52,68], bounds_subtract:[-129,49.5,-66,72], url:['https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}.png','https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}.png','https://caltopo.com/tile/modis_mp/{z}/{x}/{y}.png'], xopacity:0.8, background:'ARCGIS_AERIAL' }
		...
		,{ id:'MX_CALTOPO', menu_order:13.0, menu_name:'mx: Topo (CalTopo)', description:'North America topographic maps from CalTopo', credit:'Topo maps from <a target="_blank" href="http://www.caltopo.com/">CalTopo.com<'+'/a>', error_message:'CalTopo topo tiles unavailable', min_zoom:8, max_zoom:16, country:'mx', bounds:[-118,14.4,-86,32.72], bounds_subtract:[-99.08,26.43,-86,32.72], url:'https://caltopo.s3.amazonaws.com/topo/{z}/{x}/{y}.png' }
		,{ id:'MX_CALTOPO_RELIEF', menu_order:13.01, menu_name:'mx: Topo+relief', description:'North America relief-shaded topo from CalTopo', credit:'Topo+relief tiles from <a target="_blank" href="http://www.caltopo.com/">CalTopo.com<'+'/a>', error_message:'CalTopo tiles unavailable', min_zoom:8, max_zoom:16, country:'mx', bounds:[-118,14.4,-86,32.72], bounds_subtract:[-99.08,26.43,-86,32.72], url:'https://caltopo.s3.amazonaws.com/topo/{z}/{x}/{y}.png', foreground:'US_CALTOPO_HILLSHADING', foreground_opacity:[0.20] }
		...
		,{ id:'CH_SWISS_AERIAL', menu_order:32.21, menu_name:'ch: Swiss aerial photos', credit:'Imagery from <a target="_blank" href="https://geo.admin.ch/">geo.admin.ch</a>', min_zoom:7, max_zoom:20, country:'ch', bounds:[5,45,11,48], tile_size:256, url:'https://wmts102.geo.admin.ch/1.0.0/ch.swisstopo.swissimage/default/current/3857/{z}/{x}/{y}.jpeg' }
		,{ id:'CH_SWISS_TOPO', menu_order:32.22, menu_name:'ch: Swiss Topo maps', credit:'Maps from <a target="_blank" href="https://geo.admin.ch/">geo.admin.ch</a>', min_zoom:7, max_zoom:19, country:'ch', bounds:[5,45,11,48], tile_size:256, url:'https://wmts100.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-farbe/default/current/3857/{z}/{x}/{y}.jpeg' }
		...
		,{ id:'US_MOBILE_VERIZON', menu_order:0, menu_name:'us: mobile data: Verizon', description:'Verizon Wireless data coverage', credit:'Coverage tiles from FCC/ArcGIS', error_message:'Mobile coverage tiles unavailable', min_zoom:1, max_zoom:13, bounds:[-125,24,-66,49.5], url:'https://tiles.arcgis.com/tiles/YnOQrIGdN9JGtBh4/arcgis/rest/services/Verizon_LTE_Data/MapServer/tile/{z}/{y}/{x}', background:'ARCGIS_RELIEF', opacity:0.4 }
		,{ id:'US_MOBILE_TMOBILE', menu_order:0, menu_name:'us: mobile data: T-Mobile', description:'T-Mobile data coverage', credit:'Coverage tiles from FCC/ArcGIS', error_message:'Mobile coverage tiles unavailable', min_zoom:1, max_zoom:13, bounds:[-125,24,-66,49.5], url:'https://tiles.arcgis.com/tiles/YnOQrIGdN9JGtBh4/arcgis/rest/services/TMobile_LTE_Data/MapServer/tile/{z}/{y}/{x}', background:'ARCGIS_RELIEF', opacity:0.4 }
		,{ id:'US_MOBILE_ATT', menu_order:0, menu_name:'mobile data: AT&T', description:'AT&T data coverage', credit:'Coverage tiles from FCC/ArcGIS', error_message:'Mobile coverage tiles unavailable', min_zoom:1, max_zoom:13, bounds:[-125,24,-66,49.5], url:'https://tiles.arcgis.com/tiles/YnOQrIGdN9JGtBh4/arcgis/rest/services/ATT_Mobility_LTE_Data/MapServer/tile/{z}/{y}/{x}', background:'ARCGIS_RELIEF', opacity:0.4 }

		,{ id:'GOOGLE_ROADMAP', menu_order:0.0, menu_name:'Google map', description:'Google street map', credit:'Map tiles from Google', error_message:'Google tiles unavailable', min_zoom:1, max_zoom:21, url:'https://mt0.google.com/vt/lyrs=m&x={x}&y={y}&z={z}' }
		,{ id:'GOOGLE_HYBRID', menu_order:0.0, menu_name:'Google hybrid', description:'Google aerial imagery with labels', credit:'Map tiles from Google', error_message:'Google tiles unavailable', min_zoom:1, max_zoom:21, url:'https://mt0.google.com/vt/lyrs=y&x={x}&y={y}&z={z}' }
		,{ id:'GOOGLE_SATELLITE', menu_order:0.0, menu_name:'Google aerial', description:'Google aerial/satellite imagery', credit:'Map tiles from Google', error_message:'Google tiles unavailable', min_zoom:1, max_zoom:21, url:'https://mt0.google.com/vt/lyrs=s&x={x}&y={y}&z={z}' }
		,{ id:'GOOGLE_TERRAIN', menu_order:0.0, menu_name:'Google terrain', description:'Google terrain map', credit:'Map tiles from Google', error_message:'Google tiles unavailable', min_zoom:1, max_zoom:21, url:'https://mt0.google.com/vt/lyrs=p&x={x}&y={y}&z={z}' }
...
function GV_Define_Background_Map_Aliases() { // these aliases should ALWAYS exist and should only be edited, not removed.
	...
	gvg.bg['KOMOOT_OSM'] = gvg.bg['OPENTOPOMAP'];
	gvg.bg['ROADMAP_DESATURATED'] = gvg.bg['ARCGIS_RELIEF'];

	gvg.bg['GV_DEFAULT'] = gvg.bg['OPENSTREETMAP'];
}

if (gv_options.marker_clustering) {
	var mco = (gv_options.marker_clustering_options) ? gv_options.marker_clustering_options : [];
	mco.largeScale = (mco.largeScale) ? mco.largeScale : 1.25;
	mco.smallScale = (mco && mco.smallScale) ? mco.smallScale : 0.8;
	var mcir = mco.iconRadius = (mco && mco.iconRadius) ? mco.iconRadius : 20;
	var mcir_l = mco.iconRadiusLarge = mcir * mco.largeScale;
	var mcir_s = mco.iconRadiusSmall = mcir * mco.smallScale;
	...
	document.writeln('			.marker-cluster-large { background-clip:padding-box; border-radius:'+mcir_l+'px; }');
	document.writeln('			.marker-cluster-small { background-clip:padding-box; border-radius:'+mcir_s+'px; }');
	...
	document.writeln('			.marker-cluster-large div { width:'+(mcir_l*1.5)+'px; height:'+(mcir_l*1.5)+'px; margin-left:'+(mcir_l*0.25)+'px; margin-top:'+(mcir_l*0.25)+'px; text-align:center; border-radius:'+(mcir_l*0.75)+'px; font:12px Verdana, Arial, Helvetica, sans-serif; }');
	document.writeln('			.marker-cluster-small div { width:'+(mcir_s*1.5)+'px; height:'+(mcir_s*1.5)+'px; margin-left:'+(mcir_s*0.25)+'px; margin-top:'+(mcir_s*0.25)+'px; text-align:center; border-radius:'+(mcir_s*0.75)+'px; font:12px Verdana, Arial, Helvetica, sans-serif; }');
	document.writeln('			.marker-cluster span { line-height:'+(mcir*1.5)+'px; }');
	document.writeln('			.marker-cluster-large span { line-height:'+(mcir_l*1.5)+'px; }');
	document.writeln('			.marker-cluster-small span { line-height:'+(mcir_s*1.5)+'px; }');
...
function GV_Define_Styles() {
	...
	document.writeln('			.gv_modal_content { min-width:200px; min-height:100px; padding:12px; overflow:auto; background-color:white; border:1px solid #006600; box-shadow:6px 6px 10px #666666; text-align:left; font-size:13px; font-family:Verdana,sans-serif; }');
