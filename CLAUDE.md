...
- [ ] Epic 10: Preprocessing & watcher test remediation — analysis-first, two-phase, multi-session. Plan: `~/.claude/plans/epic10-preprocessing-watcher-tests.md`. Phase 1 analysis at `docs/test-analysis/preprocessing-watcher-2026-04-28.md`. Phase 1.5 closed 2026-04-28 with **tests-only** contract decision (no runtime schema, no shared types) — full reasoning at the top of the analysis doc. Escaped-bug log at `docs/test-analysis/escaped-bugs.md` accumulates evidence for any future restructure decision. Old monolithic plan preserved at `~/.claude/plans/epic10-testing-remediation-superseded-2026-04-27.md`.
...
- [ ] Epic 13: Library test remediation — analysis + cleanup + new tests for library code, modelled on Epic 10. Mobile-only library UI bug class lives here (mobile emulation as Playwright project). No plan file yet. Carry-over from Epic 10 Phase 2 Session A: gap 2 (`metadata.attributeRanges` → `LayerManager._ranges` initial-load consumption) was deferred here because the natural test surfaces an `AttributeLegend` recompute-vs-metadata precedence question that's better handled as an L-Unit test of `LayerManager.addLayers`. Analysis: `docs/test-analysis/preprocessing-watcher-2026-04-28.md` §3 area 1 #2.
