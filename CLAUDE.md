...
- [x] Epic 9: Preprocessing output guarantee — atomic writes, unlink-on-failure, crash cleanup. Watcher *tests* split out to Epic 10 after a mid-epic audit found foundational test-strategy problems. Plan: `~/.claude/plans/epic9-preprocessing-guarantee.md`. (Note: the `--status-file` test-instrumentation flag added in Epic 9 was never used; removed in Epic 11.)
...
- [x] Epic 11: Contract via capability map — closed 2026-05-18. `CAPABILITIES` constant in [src/contract/capabilities.ts](src/contract/capabilities.ts) is the single source of truth for the list of render capabilities; entries declare runtime predicates (from [src/contract/checks.ts](src/contract/checks.ts)) that describe what the renderer reads from a track's `properties`. `ColourAttribute = keyof typeof CAPABILITIES`, so capability ids and the renderer's attribute keys cannot drift. The validator in [src/contract/validate.ts](src/contract/validate.ts) iterates `CAPABILITIES` to check input plus runs `@mapbox/geojsonhint` for RFC 7946 envelope compliance; preprocessing calls it before atomic-rename ([build-trip-data.js](preprocessing/build-trip-data.js)). Validator ships as a separate bundle `dist/contract.js` (rolled up via [rollup.contract.config.js](rollup.contract.config.js), `geojsonhint` bundled in with `require.main === module` neutralised at build-time) — no MapLibre or DOM. Renderer dispatch was NOT routed through `CAPABILITIES`: `ColorRamps` switches remain by design; the library imports types + capabilities for the single-source-of-truth on the *list*, but the renderer's per-attribute logic stays in `ColorRamps`. Wiring test for the validator-rejection branch deferred to Epic 12 (requires refactoring `build-trip-data.js` to export `main()`). **Pivoted 2026-05-16** from original "schema as shared contract" approach (top-level `schema/` folder + valibot validator with inferred types) which was shown not to deliver mechanical drift prevention. Library robust error handling at iteration sites is OUT OF SCOPE; sits in Epic 14. Plan: `~/.claude/plans/epic11-capability-map-contract.md`. Superseded plan: `~/.claude/plans/epic11-schema-shared-contract-superseded.md`.
- [x] Epic 12: Preprocessing improvements — closed 2026-05-26 on `epic/preprocessing-improvements`. Shipped: Improvement 1 (yaml ergonomics + `defaultVisible`→`hidden` rename), Improvement 2 (configurable `ignore:` with default list, watched + scanned consistently), Improvement 3 (`--name` flag wired through demo title/toolbar), plus a build-log format revamp (one-line `[OK]`/`[FAIL]` with grouped counts and runtime, `[BUILD] Starting` at pipeline boundary with file-event cause, async build with in-flight coalescing, validator failure → input filename mapping, `NOMADPATH_WATCH_DEBUG`-gated watch logging), and several adjacent fixes (tilde expansion in CLI args, output file ignored during input collection, `CONFIG_FILE`/`OUTPUT_FILE` constants). Deferred items moved to Future blocks below — they need more thinking and will likely be picked up as parts of other epics. Memory: `project_epic12_configurable_preprocessing.md`. Original plan: `~/.claude/plans/epic12-preprocessing-improvements.md`.
...
- [ ] Epic 14: Library test remediation — analysis + cleanup + new tests for library code, modelled on Epic 10. Mobile-only library UI bug class lives here (mobile emulation as Playwright project). No plan file yet. Carry-overs: (1) from Epic 10 Phase 2 Session A: the `metadata.attributeRanges` → `LayerManager._ranges` initial-load consumption test was deferred here because the natural test surfaces an `AttributeLegend` recompute-vs-metadata precedence question that's better handled as an L-Unit test of `LayerManager.addLayers`. (2) from Epic 11: `MobileConfig` (in `src/data/types.ts`) is part of the public `TravelMapConfig` surface but no consumer reads it — decide whether to wire it up or remove it. Analysis: `docs/test-analysis/preprocessing-watcher-2026-04-28.md` §3 area 1 #2.
- [x] Epic: Library profiling instrumentation — closed 2026-06-16 on `epic/profiling` (off `master`, opened 2026-06-13). Shipped: per-phase load timing (`nomadpath.Initial load/{Load trip data, Build segments, Add to map, Mount UI}`) and per-action timing (`nomadpath.Colour change`, `nomadpath.Toggle track`), each carrying a **sync time**, a **to-first-paint** time (mutation → first MapLibre `render` event, via `measureToFirstFrame`), and a **visible-segment count** (`detail.segments`, the proxy for GPU paint cost). Surfaced by a demo-only hierarchical widget (`harness/demo/profiling-widget.js`) and an **assertion** perf test (`harness/perf/perf.spec.ts`) that drives the real UI and asserts the measures fire (the regression net). **Pivoted from the original spec** after a Stage 0 spike (`docs/test-analysis/profiling-styledirty-spike-2026-06-15.md`) proved `_styleDirty`/events can't track the true ~2s GPU settle (only pixel quiescence can, unmeasurable live) — so the deliverable is three cheap per-action numbers, NOT a single end-to-end latency. **Measure-name grammar**: `/` nests rows (phase under group), `.` marks a column sub-measure (`.firstFrame`); names are prose, shown verbatim by the widget (no label map). **Action seam is the UI event handler** (`AttributeLegend` dropdown, `TrackLegend` single + day-group checkboxes via `profileAction` in `MapEngine.ts`), NOT `LayerManager`'s storm-called mutators — so a measure covers the whole action (filter + recompute + repaint + scale refresh), not one inner step. Hard constraint MET: profiling is *absent* from `dist/nomad-path.js` via the `NOMADPATH_PROFILING` flag in `src/profiling.ts` (`profile`/`profileAsync`; `profile` takes an optional `detail` THUNK evaluated after the action) + `@rollup/plugin-replace` per output + terser DCE → `dist/nomad-path.js` (prod, stripped) and `dist/nomad-path.profiling.js` (demo + perf). Enforced by `scripts/check-prod-clean.sh` (greps our profiling strings; vendored MapLibre `performance.*`/`once("render")` are false positives, excluded). `demo/` moved to `harness/demo/`; perf harness in `harness/perf/`. Bugs found+fixed while building (all by hand, then netted by the perf test): wrong measurement layer, stale segment count (detail evaluated before the action — fixed via thunk), unprofiled day-group toggle, missing Build-segments detail. Spec: [docs/superpowers/specs/2026-06-13-library-profiling-design.md](docs/superpowers/specs/2026-06-13-library-profiling-design.md). Plans: `~/.claude/plans/i-ve-switched-to-plan-whimsical-brooks.md` (executed), superseded `~/.claude/plans/i-m-working-on-profiling-peppy-clarke.md`. Memory: `project_profiling_styledirty_spike.md`, `feedback_profiling_action_seam_layer.md`. Sequenced BEFORE the packaging refactor (`epic/refactor`); provides the measurement foundation for the Future "Performance improvements" entry.
- [ ] Future: Preprocessing items deferred from Epic 12 — these need more thinking and will likely be picked up as parts of other epics rather than as a single epic. Items: chronological output sort (move library's defensive sort into preprocessing); `main(argv)` refactor of `build-trip-data.js` (unblocks the validator-rejection wiring test deferred from Epic 11); Gap 8 design (duplicate `(day, name)` track ID collision — see Epic 12 original plan §"Defects surfaced by Epic 10 analysis"); Gap 12 design (POI with empty/missing category); sun-angle discrete bucketing (gated on Future performance epic). Same-date flight grouping is subsumed by the open `epic/flight-day-rework` sub-branch (see entry below). Analysis input: `docs/test-analysis/preprocessing-watcher-2026-04-28.md` §3 area 1.
- [ ] Future: Flight day rework — sub-epic branched off `epic/preprocessing-improvements` at commit `e195c96`; left open with broken WIP at HEAD (`88dd26d`). Clean commits below the WIP introduce `arrivalDay`, drop `crossDay` from the contract, unify `assignDay`, and sort by ISO day + first-point time. The library-side track-ID overhaul (`${day}::${name}::${index}` minted once at ingest, cached on `NomadPath`) is unfinished — design is in the memory file. Resume by deciding discard-and-restart vs salvage. Subsumes the original Epic 12 "same-date flight grouping" item (same-date flights become ordinary rows in their day group; cross-day flights render between day groups). Memory: `project_flight_day_rework.md`. Plan: `~/.claude/plans/i-m-working-on-this-peaceful-oasis.md`. Branch: `epic/flight-day-rework`.
- [ ] Future: Watcher test harness — carries the watcher-lifecycle invariant tests (atomic write, unlink-on-failure, crash cleanup, SIGINT/SIGTERM, orphan reaping). Originally sequenced in front of Epic 12 but Epic 12 closed without it; sequence in front of whichever future epic next touches the watcher surface (likely Flight day rework or the deferred-preprocessing-items entry above). The harness will observe rebuild completion via stdout (`[BUILD] Starting` → `[OK]`/`[FAIL]` lines, post Epic 12's log revamp) plus filesystem state — the `--status-file` flag the analysis doc discusses was removed in Epic 11. No plan file yet. Analysis input: `docs/test-analysis/preprocessing-watcher-2026-04-28.md` §3 area 3 (Epic 9 invariants).
...
## Current Status
- Epic 9 closed 2026-04-26 with preprocessing-side fixes: atomic temp+rename writes, unlink-on-failure on every non-success exit path, `-o`/`-p` flags, crash cleanup of orphaned serve children. (The `--status-file` test-instrumentation flag was added in Epic 9 but never used by any caller; removed in Epic 11.)
- Tests: unit + library + e2e all green via `npm run test:all`. Watcher tests pending a future epic (sequenced in front of the next preprocessing/watcher epic).
...
- Epic 11 (contract via capability map) closed 2026-05-18 on branch `epic/schema-shared-contract` (branch name kept for git continuity). Capability map + validator + preprocessing wiring all in place; 317 unit + 108 library + 9 e2e tests green; 305-track Hawaii smoke build accepted by validator. Renderer dispatch was deliberately NOT routed through `CAPABILITIES` — see the Epic 11 entry above. Validator-rejection-branch wiring test deferred to Epic 12 — Epic 12 closed without it; now lives in the deferred-preprocessing-items Future entry.
- Epic 12 (preprocessing improvements) closed 2026-05-26 on `epic/preprocessing-improvements`. Three improvements (yaml ergonomics + `defaultVisible`→`hidden` rename, configurable `ignore:`, `--name` in demo) plus a build-log format revamp shipped. Remaining items from the original plan (chronological sort, `main(argv)` refactor, Gap 8/12, sun-angle bucketing) moved to a deferred Future entry — they need more thinking and will likely be picked up as parts of other epics. Same-date flight grouping is subsumed by the open `epic/flight-day-rework` sub-branch, also moved to a Future entry.
...
Three test categories in place; a watcher harness is pending a future epic (sequenced in front of the next preprocessing/watcher epic).
...
Scripts: `build:lib` / `build:data` / `build:demo` / `demo` / `watch` / `git:enable` / `typecheck` / `lint` / `lint:fix` / `format` / `test:unit` / `test:coverage` / `test:unit:watch` / `test:library` / `test:e2e` / `test:perf` / `test:all` / `test:all:browsers` / `test:library:debug` / `test:e2e:debug` / `test:library:install` / `clean`
`test:perf` (profiling epic): builds the profiling bundle + perf fixture, runs `harness/perf/perf.spec.ts` (UI-driven; asserts the profiling measures fire + prints durations). NOT part of `test:all`. Prod-cleanliness gate: `npm run build:lib && bash scripts/check-prod-clean.sh`.
