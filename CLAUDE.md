...
- [ ] Epic 12: Preprocessing improvements — in progress on `epic/preprocessing-improvements`. Improvements 1 (yaml ergonomics + `defaultVisible`→`hidden` rename) and 2 (configurable `ignore:` with default list, watched + scanned consistently) landed. Improvement 3 (`--name` demo/watch behaviour) next — plan stub: `~/.claude/plans/epic12-improvement3-name-demo-watch.md`. Other items still on the original plan: chronological output sort, same-date flight grouping, `main(argv)` refactor, Gap 8 / Gap 12 design. Memory: `project_epic12_configurable_preprocessing.md`. Original plan: `~/.claude/plans/epic12-preprocessing-improvements.md`. Depends on Epic 11's contract foundation.
...
- [ ] Future: Watcher test harness — sequenced in front of Epic 12. Carries the watcher-lifecycle invariant tests (atomic write, unlink-on-failure, crash cleanup, SIGINT/SIGTERM, orphan reaping) and Epic-12-specific watcher behaviour tests once Epic 12's scope crystallises. Split out from Epic 10 because building a bullet-proof harness against today's surface would be invalidated by Epic 12's not-yet-known config/behaviour changes. The harness will need to observe rebuild completion via stdout (`[watch] Done.` / `[watch] Build failed`) plus filesystem state, since the `--status-file` flag the analysis doc discusses was removed in Epic 11. No plan file yet. Analysis input: `docs/test-analysis/preprocessing-watcher-2026-04-28.md` §3 area 3 (Epic 9 invariants).
