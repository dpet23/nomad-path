...
- [ ] Epic 11: Contract via capability map — library declares its render capabilities as a runtime `CAPABILITIES` constant in `src/contract/`; renderer's internal dispatch goes through that constant (so removing a capability makes the renderer unreachable, mechanical drift prevention); validator (also in `src/contract/`, built to a separate `dist/contract.{esm,cjs}.js` artifact) iterates the same constant to check input; preprocessing imports the built validator and calls it before atomic-rename. **Pivoted 2026-05-16** from original "schema as shared contract" approach (top-level `schema/` folder + valibot validator with inferred types) which was shown not to deliver mechanical drift prevention. Library robust error handling at iteration sites is OUT OF SCOPE; sits in Epic 14. Live plan: `~/.claude/plans/epic11-capability-map-contract.md`. Superseded plan: `~/.claude/plans/epic11-schema-shared-contract-superseded.md`.
...
- Epic 11 (contract via capability map) IN PROGRESS on branch `epic/schema-shared-contract` (name kept for git continuity). Originally scoped 2026-05-02 as schema-based; pivoted 2026-05-16 to capability-map approach after schema-side implementation was shown not to deliver mechanical drift prevention. Step 0 (revert of rejected schema work) complete at commit `f0f3459`. Steps (a)-(d) remaining. Live plan: `~/.claude/plans/epic11-capability-map-contract.md`. Resume by reading the plan in full.
