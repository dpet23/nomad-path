...
- [ ] Epic: Library profiling instrumentation — on `epic/profiling` (off `master`, opened 2026-06-13). Per-phase `performance.mark`/`measure` timing in the library (geojson parse, track ingestion, colour render, colour re-render), surfaced by a demo-only widget, with a characterization perf test as the foundation for a future perf-regression net. Hard constraint: profiling code must be *absent* from the prod build artifact (`dist/nomad-path.js`) — achieved via a single `NOMADPATH_TIMING` flag in `src/profiling.ts` (closure wrappers `profile`/`profileAsync`), `@rollup/plugin-replace` set per output + terser DCE, emitting two library-owned bundles: `dist/nomad-path.js` (prod, stripped) and `dist/nomad-path.profiling.js` (demo + perf test). New code lands in a plain `harness/` subfolder (NOT a workspace package): `demo/` moves to `harness/demo/` (modified anyway), perf test is new; `test/integration`, `test/e2e`, `watch`, `git-rebuilds` stay put. Sequenced BEFORE the packaging refactor (`epic/refactor`) and structured so every piece relocates cleanly into it. 5 stages, each independently green, executed one-per-session via executing-plans. Spec: [docs/superpowers/specs/2026-06-13-library-profiling-design.md](docs/superpowers/specs/2026-06-13-library-profiling-design.md). Plan: `~/.claude/plans/i-m-working-on-this-zazzy-sutherland.md`. Relates to the Future "Performance improvements" entry (provides its measurement foundation).
